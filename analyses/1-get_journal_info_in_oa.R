#' Retrieve DAFNEE Journals metadata in OA database
#' 


## Read DAFNEE database ----

dafnee_journals <- read.csv(here::here("data", "raw-data", 
                                       "DAFNEE-20241022.csv"))
...
dafnee_journals$"journal_clean" <- 
  gsub("Proceedings of the National Academy of Sciences of the USA", 
...
dafnee_journals <- merge(x    = dafnee_journals, 
                         y    = oa_data, 
                         by.x = "journal_clean",
                         by.y = "journal",
                         all  = TRUE)
...
round(100 * sum(is.na(dafnee_journals$"oa_works_count")) / nrow(dafnee_journals), 0)


## Export data ----

write.csv(dafnee_journals, here::here("data", "derived-data", 
